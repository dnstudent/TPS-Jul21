...
class DirectRnn(PollutionNet, keras.Sequential):
    ...
    """
    def __init__(self, n_features, n_targets, rnn_class, n_recursive_units, input_days, output_days, offset_hours=None, n_intermediate_fc_units=None, rnn_kwargs=None, dense_kwargs=None, *args, **kwargs):
        ...
        for i, n_units in enumerate(n_recursive_units):
        ...
        for n_units in n_intermediate_fc_units:
            self.add(keras.layers.Dense(n_units, name=f"{self.name}_intermediate_dense_{n_units}", **dense_kwargs))
        self.out = keras.layers.Dense(n_targets, name=f"{self.name}_out", **dense_kwargs)
        self.add(self.out)
